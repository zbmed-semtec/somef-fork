        ...
        for file in files:
            if file.endswith((".ipynb", ".rmd", '.Rmd')):
                ...
                try:
                    ...
                    print(
                        f"Error reading notebook file {notebook_path}: {str(e)}")
                    pass
    ...
    for root, dirs, files in os.walk(path_repo):
        ...
        for file in files:
            ...
            if file.endswith(constants.code_extensions):
                ...
                if not ontology:
                    ontology = is_file_ontology(
                        os.path.join(path_repo, file_path))
    return ontology
                ...
                if repo_relative_path == ".":
                    try:
                        ...
                        with open(os.path.join(dir_path, filename), "r", encoding="utf-8") as data_file:
                            ...
                            try:
                                cmd_match2 = re.search(
                                    pattern_commandline, data_file_text)
                                cmd_match3 = re.search(
                                    pattern_cmd_arg, data_file_text)
                                cmd_match4 = re.search(
                                    pattern_cmd_arg2, data_file_text)
                ...
                if repo_relative_path == ".":
                    try:
                        ...
                        with open(os.path.join(root, file), "r") as readme_file:
                            ...
                            pattern = r'([^.?!]*(?:\b|\W){}(?:\b|\W)[^.?!]*[.?!])'.format(
                                '|'.join(map(re.escape, title_words)))
                            sentences = re.findall(
                                pattern, readme_contents, flags=re.IGNORECASE)
                            ...
                            match_md = re.findall(
                                pattern_md, readme_contents, re.IGNORECASE)
                            match_rst = re.findall(
                                pattern_rst, readme_contents, re.MULTILINE | re.IGNORECASE)
...
def check_name(filename):
    ...
    programming_language_extensions = (".py", ".java", ".cpp", ".c", ".php",
                                       ".rb", ".js", ".html", ".css", ".go", ".swift", ".scala", ".pl", ".ipynb")
